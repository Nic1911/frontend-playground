...
export const initScrollMagicInnovation = () => {
	const innovationSec = document.getElementsByClassName(
		"ns-innovation__section"
	);
	const controller = new ScrollMagic.Controller();

	for (let i = 0; i < innovationSec.length; i++) {
		const element = innovationSec[i];
		const panelImage = element.getElementsByClassName(
			"ns-innovation__image"
		)[0];
		const panelWrap = element.getElementsByClassName(
			"ns-innovation__panelWrap"
		)[0];
		const panelMain = element.getElementsByClassName(
			"ns-innovation__panel"
		)[0];
		const panelContent = element.getElementsByClassName(
		...
		const bordersInside = element.getElementsByClassName(
			"ns-innovation__bordersInside"
		)[0];
		const bordersInsideLateral = bordersInside.querySelectorAll(
			".ns-innovation__border--right, .ns-innovation__border--left"
		);
		const bordersInsideTopBottom = bordersInside.querySelectorAll(
			".ns-innovation__border--top, .ns-innovation__border--bottom"
		);
		/* TIMELINE AND TWEEN */
		...
		const panelImageParallax = TweenMax.to(
			panelImage,
			1,
			{
				y: 100,
